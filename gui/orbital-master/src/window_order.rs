...
impl WindowOrder {
    ...
    pub(crate) fn focus_order(&self) -> impl Iterator<Item = WindowId> + use<'_> {
        self.focus_order.iter().copied()
    }

    pub(crate) fn iter_front_to_back(&self) -> impl Iterator<Item = WindowId> + use<'_> {
        self.zbuffer.iter().map(|&(id, _, _)| id)
    }

    pub(crate) fn iter_back_to_front(&self) -> impl Iterator<Item = (WindowId, bool)> + use<'_> {
        self.zbuffer
            .iter()
