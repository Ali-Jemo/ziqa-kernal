...
pub fn handle(ctx: &mut SyscallContext) -> Option<Result<u64, AbiError>> {
    match ctx.number {
        ...
        super::nr::SYS_SIGALTSTACK => Some(sys_sigaltstack(ctx)),
        _ => None,
    }
}

fn sys_sigaltstack(_ctx: &mut SyscallContext) -> Result<u64, AbiError> {
    // Zig's Linux startup installs an alternate signal stack when available.
    // ZiqaKernel does not deliver Linux signals on user stacks yet, so accepting
    // the request as a no-op is safer than returning ENOSYS and tripping
    // userspace startup panic paths.
    Ok(0)
}
