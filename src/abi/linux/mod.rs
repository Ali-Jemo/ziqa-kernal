...
) -> x86_64::structures::paging::mapper::OffsetPageTable<'static> { unsafe {
...
fn sys_writev(ctx: &mut SyscallContext) -> Result<u64, AbiError> {
    ...
    for i in 0..iovcnt.min(16) {
        ...
        // Do this before taking the serial lock: user copies can fault in
        // demand-paged ELF data, and the page-fault logger also writes serial.
        let iov_offset = i * 16;
...
fn sys_open(ctx: &mut SyscallContext) -> Result<u64, AbiError> {
    ...
    // FIX 1: Translate Redox-style scheme paths: /scheme/<scheme>/<resource> → <scheme>:<resource>
    // This translates "/scheme/input/consumer" → "input:consumer"
    ...
    let is_known = known_path(&translated_path)
        || matches!(
            translated_path.as_str(),
            "/dev/null"
                | "/dev/zero"
                | "/dev/random"
                | "/dev/urandom"
    ...
    if o_creat {
        ...
        if !vfs.exists(&translated_path) {
            vfs.create(&translated_path);
        }
    }
    ...
    if is_known
        ...
        || crate::fs::vfs::VFS
            ...
            .is_ok()
    {
        let fd = ctx.process.fds.alloc_file(translated_path.as_bytes(), flags).ok_or(AbiError::Other("EMFILE"))?;
        return Ok(fd as u64);
    }
