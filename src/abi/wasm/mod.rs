...
impl AbiPlugin for WasmAbiPlugin {
    ...
    fn load(&self, _binary: &[u8], process: &mut Process) -> Result<(), AbiError> {
        ...
        process.entry_point = crate::memory::VirtAddr::new(wasm_interpreter_entry as *const () as u64);
...
fn parse_wasm(data: &[u8]) -> Result<WasmInstance, &'static str> {
    ...
    while offset < data.len() {
        ...
        let sec_len = read_leb128(data, &mut offset).map_err(|_| "Failed to read section length")? as usize;
        ...
        match sec_id {
            1 => { // Type Section
                let count = read_leb128(data, &mut offset).map_err(|_| "Failed to read type count")? as usize;
                for _ in 0..count {
                    ...
                    let param_count = read_leb128(data, &mut offset).map_err(|_| "Failed to read param count")? as usize;
                    offset += param_count; // skip param types
                    
                    let ret_count = read_leb128(data, &mut offset).map_err(|_| "Failed to read ret count")? as usize;
            ...
            2 => { // Import Section
                let count = read_leb128(data, &mut offset).map_err(|_| "Failed to read import count")? as usize;
                for _ in 0..count {
                    let mod_len = read_leb128(data, &mut offset).map_err(|_| "Failed to read mod length")? as usize;
                    let module_name = core::str::from_utf8(&data[offset..offset+mod_len]).map_err(|_| "Invalid module name")?;
                    offset += mod_len;
                    
                    let field_len = read_leb128(data, &mut offset).map_err(|_| "Failed to read field length")? as usize;
                    let field_name = core::str::from_utf8(&data[offset..offset+field_len]).map_err(|_| "Invalid field name")?;
                    offset += field_len;
                    ...
                    if kind == 0x00 { // Function import
                        let type_idx = read_leb128(data, &mut offset).map_err(|_| "Failed to read type index")? as usize;
                        ...
                        let _min = read_leb128(data, &mut offset).map_err(|_| "Failed to read table min")?;
                    } else if kind == 0x02 { // Memory import
                        let _flags = data[offset]; offset += 1;
                        let min = read_leb128(data, &mut offset).map_err(|_| "Failed to read memory min")? as usize;
                        memory_size_pages = min;
                    } else {
                        let _type = data[offset]; offset += 1;
                        let _mutability = data[offset]; offset += 1;
                    }
                }
            }
            3 => { // Function Section
                let count = read_leb128(data, &mut offset).map_err(|_| "Failed to read func count")? as usize;
                for _ in 0..count {
                    let type_idx = read_leb128(data, &mut offset).map_err(|_| "Failed to read type index")? as usize;
            ...
            5 => { // Memory Section
                let count = read_leb128(data, &mut offset).map_err(|_| "Failed to read memory count")? as usize;
                for _ in 0..count {
                    ...
                    memory_size_pages = min;
                    if flags & 1 != 0 {
                        let _max = read_leb128(data, &mut offset).map_err(|_| "Failed to read memory max")?;
                    }
                }
            }
            7 => { // Export Section
                let count = read_leb128(data, &mut offset).map_err(|_| "Failed to read export count")? as usize;
                for _ in 0..count {
                    let name_len = read_leb128(data, &mut offset).map_err(|_| "Failed to read export name length")? as usize;
                    let name = core::str::from_utf8(&data[offset..offset+name_len]).map_err(|_| "Invalid export name")?;
                    offset += name_len;
                    
                    let kind = data[offset];
                    offset += 1;
                    let idx = read_leb128(data, &mut offset).map_err(|_| "Failed to read export index")? as usize;
            ...
            8 => { // Start Section
                let idx = read_leb128(data, &mut offset).map_err(|_| "Failed to read start index")? as usize;
                start_func_idx = Some(idx);
            }
            10 => { // Code Section
                let count = read_leb128(data, &mut offset).map_err(|_| "Failed to read code count")? as usize;
                for _ in 0..count {
                    let body_size = read_leb128(data, &mut offset).map_err(|_| "Failed to read body size")? as usize;
                    let body_end = offset + body_size;
                    
                    let locals_count = read_leb128(data, &mut offset).map_err(|_| "Failed to read locals count")? as usize;
                    let mut decls = Vec::new();
                    for _ in 0..locals_count {
                        let count = read_leb128(data, &mut offset).map_err(|_| "Failed to read local count")? as u32;
                        let val_type = data[offset];
                        offset += 1;
                        decls.push(WasmLocalDecl { count, _val_type: val_type });
                    }
            ...
            11 => { // Data Section
                let count = read_leb128(data, &mut offset).map_err(|_| "Failed to read data count")? as usize;
                for _ in 0..count {
                    let flags = read_leb128(data, &mut offset).map_err(|_| "Failed to read data flags")?;
                    if flags != 0 {
                        // ignore flags
                    }
                    if data[offset] != 0x41 {
                    ...
                    offset += 1;
                    let mem_off = read_leb128_signed(data, &mut offset).map_err(|_| "Failed to read offset value")? as usize;
                    if data[offset] != 0x0B {
                        return Err("Expected end opcode after offset expression");
                    }
                    offset += 1;
                    
                    let data_len = read_leb128(data, &mut offset).map_err(|_| "Failed to read data segment length")? as usize;
...
fn skip_instruction_args(code: &[u8], pc: &mut usize) -> Result<(), &'static str> {
    if *pc >= code.len() { return Ok(()); }
    let op = code[*pc];
    *pc += 1;
    match op {
        0x02 | 0x03 | 0x04 => { // block, loop, if
            if *pc < code.len() { *pc += 1; } // skip block type
        }
        0x0C | 0x0D => { // br, br_if
...
fn execute_function(
    ...
    let func_info = match &instance.functions[func_idx] {
        ...
        WasmFunction::Internal { locals_decl, code, .. } => WasmFunctionInfo::Internal {
        ...
        WasmFunctionInfo::Imported { module, name } => {
            ...
            if module == "wasi_snapshot_preview1" && name == "proc_exit" {
                ...
                let current_pid = x86_64::instructions::interrupts::without_interrupts(|| {
                    crate::process::scheduler::SCHEDULER.lock().current_task().map(|t| t.pid)
                });
                if let Some(pid) = current_pid {
                    x86_64::instructions::interrupts::without_interrupts(|| {
                        crate::process::scheduler::SCHEDULER.lock().exit_process(pid, status);
                    });
                }
                return Ok(Some(0));
            }
            ...
            while pc < code.len() {
                ...
                match op {
                    ...
                    0x02 => { // block
                        ...
                        control_stack.push(ControlBlock { op: 0x02, start_pc: pc, end_pc });
                    }
                    0x03 => { // loop
                        let _type = code[pc];
                        pc += 1;
                        let end_pc = find_matching_end(&code, pc)?;
                        control_stack.push(ControlBlock { op: 0x03, start_pc: pc, end_pc });
                    }
                    0x04 => { // if
                        ...
                        if cond != 0 {
                            control_stack.push(ControlBlock { op: 0x04, start_pc: pc, end_pc });
                        } else {
                            if let Some(epc) = else_pc {
                                pc = epc + 1;
                                control_stack.push(ControlBlock { op: 0x05, start_pc: pc, end_pc });
                            } else {
                                pc = end_pc;
                            }
                        }
                    ...
                    0x0C => { // br
                        let mut offset = pc;
                        let label_idx = read_leb128(&code, &mut offset).map_err(|_| "Failed to read br label index")? as usize;
                    ...
                    0x0D => { // br_if
                        let mut offset = pc;
                        let label_idx = read_leb128(&code, &mut offset).map_err(|_| "Failed to read br_if label index")? as usize;
                        pc = offset;
                    ...
                    0x10 => { // Call
                        let mut offset = pc;
                        let target_idx = read_leb128(&code, &mut offset).map_err(|_| "Failed to read call index")? as usize;
                        pc = offset;
                    ...
                    0x20 => { // Local.get
                        let mut offset = pc;
                        let idx = read_leb128(&code, &mut offset).map_err(|_| "Failed to read local index")? as usize;
                        pc = offset;
                    ...
                    0x36 => { // i32.store
                        let mut offset = pc;
                        let _align = read_leb128(&code, &mut offset).map_err(|_| "Failed to read store alignment")?;
                        let store_off = read_leb128(&code, &mut offset).map_err(|_| "Failed to read store offset")? as usize;
                        pc = offset;
                        ...
                        instance.memory[target_addr..target_addr+4].copy_from_slice(&val.to_le_bytes());
                    }
                    0x41 => { // i32.const
                        let mut offset = pc;
                        let val = read_leb128_signed(&code, &mut offset).map_err(|_| "Failed to read i32.const")? as i32;
                        pc = offset;
                        stack.push(val);
                    }
                    0x6A => { // i32.add
    ...
    match parse_wasm(&binary) {
        Ok(mut instance) => {
            if let Some(start_idx) = instance.start_func_idx {
                println!("[WASM Runtime] Running function _start (index {})...", start_idx);
                match execute_function(&mut instance, start_idx, &[]) {
                    Ok(val) => {
                        println!("[WASM Runtime] Module executed successfully. Exit status: {:?}", val.unwrap_or(0));
...
pub const TEST_WASM: &[u8] = &[
    ...
    0x01,       // 1 import
    22, b'w', b'a', b's', b'i', b'_', b's', b'n', b'a', b'p', b's', b'h', b'o', b't', b'_', b'p', b'r', b'e', b'v', b'i', b'e', b'w', b'1',
    8,  b'f', b'd', b'_', b'w', b'r', b'i', b't', b'e',
    ...
    0x02,       // 2 exports
    6, b'm', b'e', b'm', b'o', b'r', b'y',
    0x02, 0x00, // kind: Memory, index 0
    6, b'_', b's', b't', b'a', b'r', b't',
    ...
    31,         // length 31
    b'H', b'e', b'l', b'l', b'o', b' ', b'f', b'r', b'o', b'm', b' ', b'W', b'e', b'b', b'A', b's', b's', b'e', b'm', b'b', b'l', b'y', b' ', b'(', b'W', b'A', b'S', b'I', b')', b'!', b'\n',
];
