...
fn is_user_frame(frame: &InterruptStackFrame) -> bool {
    frame.code_segment & 0x3 == 0x3
}

fn terminate_current_user_process(code: i64) -> ! {
    if let Some(pid) = crate::process::scheduler::SCHEDULER.current_pid() {
        crate::process::scheduler::SCHEDULER.exit_process(pid, code);
    }
    loop {
...
extern "x86-interrupt" fn invalid_opcode_handler(frame: InterruptStackFrame) {
    if is_user_frame(&frame) {
        println!("INVALID OPCODE in userspace at rip={:#x}", frame.instruction_pointer.as_u64());
        terminate_current_user_process(-4);
    }
...
extern "x86-interrupt" fn page_fault_handler(
    ...
    let handle_fault_for = |proc: &crate::process::Process| -> bool {
        ...
        if is_instruction_fetch && !vma.flags.executable {
            return false;
        }
        ...
        crate::memory::paging::demand_page_for_root(
    ...
    if is_user_frame(&frame) {
        terminate_current_user_process(-11);
    }
...
extern "x86-interrupt" fn syscall_handler(_frame: InterruptStackFrame) {
    ...
    if crate::process::scheduler::with_current_task(|proc| {
        matches!(
            proc.state,
            crate::process::ProcessState::Exited(_) | crate::process::ProcessState::Canceled
        )
    })
    .unwrap_or(false)
    {
