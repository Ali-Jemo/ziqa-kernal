...
fn fill_frame(frame: PhysFrame, init_data: Option<&[u8]>) {
    let po = phys_offset();
    let frame_ptr = (po + frame.start_address().as_u64()).as_mut_ptr::<u8>();
    unsafe {
...
fn map_allocated_frame(
    root_frame: Option<PhysFrame>,
    page: Page<x86_64::structures::paging::Size4KiB>,
    frame: PhysFrame,
    ...
    if let Some(root_frame) = root_frame {
        let l4 = unsafe { frame_as_page_table_mut(root_frame) };
        let mut mapper = unsafe { OffsetPageTable::new(l4, po) };
        unsafe { map_or_replace(&mut mapper, page, frame, page_flags, fa) }
    } else {
        let mut guard = KERNEL_MAPPER.lock();
        let mapper = &mut guard.as_mut().expect("KERNEL_MAPPER not initialized").mapper;
        unsafe { map_or_replace(mapper, page, frame, page_flags, fa) }
    }
}

unsafe fn map_or_replace(
...
pub fn map_elf_region(
    ...
    for page_index in 0..page_count {
        let page_addr = VirtAddr::new(start_addr + (page_index as u64 * 4096));
        let page_file_offset = page_index as u64 * 4096;

        let frame = {
            let mut fa_guard = FRAME_ALLOCATOR.lock();
            let fa = fa_guard.as_mut().expect("FRAME_ALLOCATOR not initialized");
            fa.allocate_frame().ok_or("OOM mapping ELF region")?
        };

        let init_data = if page_file_offset < file_size {
            let binary_offset = (file_offset + page_file_offset) as usize;
            if binary_offset < binary.len() {
                let copy_len = (file_size - page_file_offset)
                    .min(4096)
                    .min((binary.len() - binary_offset) as u64) as usize;
                Some(&binary[binary_offset..binary_offset + copy_len])
            } else {
                None
            }
        } else {
...
pub fn demand_page_for_root(
    ...
    map_allocated_frame(
        root_frame,
        Page::containing_address(fault_addr),
        frame,
        page_flags,
    )
    .is_ok()
...
/// Demand page using a process page table.
pub fn demand_page_for_frame(
    ...
    demand_page_for_root(Some(pt_frame), fault_addr, page_flags, init_data)
}
/// Map all Grant regions of a process into the shared kernel page table.
/// Since create_process_page_table() returns None, we use KERNEL_MAPPER.
pub fn map_process_regions(addr_space: &AddressSpace, binary_data: &[u8]) -> Result<(), &'static str> {
    let po = phys_offset();
    
    // Get mutable access to the mapper through MutexGuard
    let mut guard = KERNEL_MAPPER.lock();
    let mapper = &mut guard.as_mut().expect("KERNEL_MAPPER not initialized").mapper;
    ...
    for grant in &addr_space.grants {
        ...
        for addr in (start_addr..end_addr).step_by(4096) {
            ...
            if copy_size > 0 {
                let dst = (po + frame.start_address().as_u64()).as_mut_ptr::<u8>();
                unsafe {
