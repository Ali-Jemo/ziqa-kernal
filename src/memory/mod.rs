...
pub fn verify_user_region(addr: u64, len: usize) -> bool {
    ...
    // Check if the range is within the current task's VMAs.
    // If we are currently executing a syscall, the scheduler process lock is already
    // held by this CPU core, so we bypass the mutex to avoid recursive deadlock.
    ...
    if raw != 0 {
        ...
        proc.vmas.iter().any(|vma| {
            addr >= vma.start.as_u64() && end <= vma.end.as_u64()
        })
    } else {
        crate::process::scheduler::with_current_task(|proc| {
            proc.vmas.iter().any(|vma| {
                addr >= vma.start.as_u64() && end <= vma.end.as_u64()
            })
        }).unwrap_or(false)
