...
pub fn register_channel(id: usize, chan: Arc<Channel>) {
    // Also register in the legacy CHANNELS table for backward compat
    let mut channels = CHANNELS.lock();
    channels[id] = Some(chan.clone());
    // Register in the IPC ChannelTable — this is what send()/recv() actually use.
    // ChannelTable is 1-indexed: get(id) returns channels[id-1].
    if id > 0 && id <= MAX_CHANNELS {
        let mut table = IPC.write();
        table.channels[id - 1] = Some(chan);
    }
}
