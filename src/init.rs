...
pub fn init(boot_info: &'static bootloader::BootInfo) {
    ...
    *memory::FRAME_ALLOCATOR.lock() = Some(memory::BootInfoFrameAllocator);
    ...
    // ponytail: needed for both Orbital and games; remove cfg(games) gate
    ...
    if let Some(acpi_info) = &*drivers::acpi::ACPI_INFO.lock() {
        ...
        arch::x86_64::apic::redirect_irq(2, crate::arch::x86_64::interrupts::InterruptIndex::Timer as u8, bsp_apic_id);
