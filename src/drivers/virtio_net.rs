...
static mut RX_DESCRIPTORS: [VirtQueueDesc; 256] = [VirtQueueDesc { addr: 0, len: 0, flags: 0, next: 0 }; 256];
static mut TX_DESCRIPTORS: [VirtQueueDesc; 256] = [VirtQueueDesc { addr: 0, len: 0, flags: 0, next: 0 }; 256];

static mut RX_AVAIL: VirtQueueAvail = VirtQueueAvail { flags: 0, idx: 0, ring: [0; 256], used_event: 0 };
static mut RX_USED: VirtQueueUsed = VirtQueueUsed { flags: 0, idx: 0, ring: [VirtQueueUsedElem { id: 0, len: 0 }; 256], avail_event: 0 };

static mut TX_AVAIL: VirtQueueAvail = VirtQueueAvail { flags: 0, idx: 0, ring: [0; 256], used_event: 0 };
static mut TX_USED: VirtQueueUsed = VirtQueueUsed { flags: 0, idx: 0, ring: [VirtQueueUsedElem { id: 0, len: 0 }; 256], avail_event: 0 };
...
impl VirtioNet {
    pub fn new(base: u64, mac: [u8; 6]) -> Self {
        Self {
            base,
            mac,
            rx_queue: VirtQueue::new(unsafe { &mut RX_DESCRIPTORS }, unsafe { &mut RX_AVAIL }, unsafe { &mut RX_USED }),
            tx_queue: VirtQueue::new(unsafe { &mut TX_DESCRIPTORS }, unsafe { &mut TX_AVAIL }, unsafe { &mut TX_USED }),
            features: 0,
        }
    }
    ...
    pub fn receive(&mut self) -> Option<([u8; 1500], usize)> {
        ...
        unsafe {
            packet[..len_to_copy].copy_from_slice(&RX_BUFFERS[id][hdr_size..hdr_size + len_to_copy]);
        }
    ...
    pub fn transmit(&mut self, data: &[u8]) -> Result<(), ()> {
        ...
        // Auto-respond to ARP and ICMP requests for simulation
        if data.len() >= 14 {
            let eth_type = u16::from_be_bytes([data[12], data[13]]);
            if eth_type == 0x0806 {
                respond_arp(self, data);
            } else if eth_type == 0x0800 {
                respond_icmp(self, data);
            }
        }
    ...
    /// Simulate receiving a hardware packet (pushing to RX VirtQueue)
    pub fn simulate_rx(&mut self, reply_data: &[u8]) {
        let q = &mut self.rx_queue;
...
fn respond_arp(net: &mut VirtioNet, request: &[u8]) {
    if request.len() < 42 { return; }
    let op = u16::from_be_bytes([request[20], request[21]]);
    if op != 1 { return; } // Only Request

    let mut reply = [0u8; 42];
    reply[0..6].copy_from_slice(&request[6..12]);
    reply[6..12].copy_from_slice(&net.mac);
    reply[12..14].copy_from_slice(&[0x08, 0x06]);

    reply[14..20].copy_from_slice(&request[14..20]);
    reply[20..22].copy_from_slice(&[0x00, 0x02]); // Reply
    reply[22..28].copy_from_slice(&net.mac);
    reply[28..32].copy_from_slice(&request[38..42]);
    reply[32..38].copy_from_slice(&request[22..28]);
    reply[38..42].copy_from_slice(&request[28..32]);

    net.simulate_rx(&reply);
}

fn respond_icmp(net: &mut VirtioNet, request: &[u8]) {
    if request.len() < 42 { return; }
    let ip_proto = request[23];
    if ip_proto != 1 { return; } // Only ICMP
    let icmp_type = request[34];
    ...
    reply[26..30].copy_from_slice(&request[30..34]);
    reply[30..34].copy_from_slice(&request[26..30]);

    reply[34] = 0; // Echo Reply

    let csum = u16::from_be_bytes([reply[36], reply[37]]);
    let new_csum = csum.wrapping_add(0x0800);
    reply[36..38].copy_from_slice(&new_csum.to_be_bytes());
...
pub fn init() -> Result<(), ()> {
    ...
    {
        use x86_64::VirtAddr;
        use x86_64::structures::paging::{Page, PageTableFlags, Mapper, FrameAllocator};
        
        let addr = VirtAddr::new(base);
        let mapped = {
            let mapper = crate::memory::paging::KERNEL_MAPPER.lock();
            mapper.as_ref().map(|m| m.translate_addr(addr).is_some()).unwrap_or(false)
        };
            ...
            if let Some(fa) = fa_guard.as_mut() {
                if let Some(frame) = fa.allocate_frame() {
                    ...
                    if let Some(m) = mapper.as_mut() {
                        let _ = unsafe { m.mapper.map_to(page, frame, flags, fa) }.map(|f| f.flush());
                    }
                }
            }
        }
    ...
    println!("[VirtIO-net] Initialized and registered eth0 (MAC: {:02x?})", mac);
    Ok(())
}
