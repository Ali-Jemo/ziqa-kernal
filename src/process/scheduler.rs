...
impl Scheduler {
    ...
    pub fn spawn_redox_elf_from_vec(&self, binary: Vec<u8>) -> Option<Pid> {
        let plugin = &crate::abi::redox::REDOX_PLUGIN;
        let pid = self.pid_allocator.lock().alloc_pid();
        let entry = VirtAddr::new(0x1000000);
        let stack = VirtAddr::new(0x7FFF_FFFF_000);
    ...
    pub fn tick(&self) {
        ...
        // Diagnostic output once per second (100 ticks @ 10ms/tick)
        static LAST_LOG: core::sync::atomic::AtomicU64 = core::sync::atomic::AtomicU64::new(0);
        let ticks = self.ticks.load(Ordering::Relaxed);
        if ticks.saturating_sub(LAST_LOG.load(Ordering::Relaxed)) >= 100 {
            LAST_LOG.store(ticks, Ordering::Relaxed);
            crate::println!("[Diag Ticks] ticks={}", ticks);
            if let Some(table) = self.process_table.try_read() {
                crate::println!("[Diag] Scheduler state at {} ms:", ticks * 10);
                for (&pid, proc_arc) in table.tasks.iter() {
                    if let Some(proc) = proc_arc.try_lock() {
                        crate::println!("  PID {}: State={:?}, ABI={:?}", pid.0, proc.state, proc.abi);
                    }
                }
            }
        }
    ...
    pub fn schedule(&self) {
        ...
        loop {
            ...
            let new_state = { new_proc_arc.lock().state };
            if old_pid == Some(new_pid) && new_state == ProcessState::Running {
                if interrupts_enabled {
                    crate::arch::enable_interrupts();
                }
                return;
            ...
            // Note: old_fpu_ptr is dangerous if old_arc is dropped, 
            // but it's only used within the switch_context scope where old_arc is valid.
            let new_fpu_ptr = &new_proc_arc.lock().fpu_state as *const _;
            ...
            if old_proc_arc.is_some() || !old_proc_raw.is_null() {
                ...
                let mut dummy: u64 = 0;
                unsafe {
                // FPU already restored before switch_context
...
pub fn spawn_redox_elf_from_vec(binary: Vec<u8>) -> Option<Pid> { crate::arch::without_interrupts(|| SCHEDULER.spawn_redox_elf_from_vec(binary)) }
