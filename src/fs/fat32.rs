...
/// Collect up to `max_clusters` clusters from a chain starting at `start_cluster`.
fn collect_cluster_chain_limited(
    disk: &dyn BlockDevice,
    bpb: &Fat32Bpb,
    start_cluster: u32,
    max_clusters: usize,
...
impl File for Fat32File {
    fn read(&self, buf: &mut [u8], offset: usize) -> Result<usize, AbiError> {
        ...
        let bpc = (self.bpb.sectors_per_cluster as usize) * (self.bpb.bytes_per_sector as usize);
        let mut cluster = self.start_cluster;
        let mut skip = offset;
        while skip >= bpc {
            cluster = follow_cluster_chain(&*self.disk, &self.bpb, cluster)
                .ok_or_else(|| AbiError::Other("FAT32 chain read error"))?;
            skip -= bpc;
        }

        let nclusters = (to_read + bpc - 1) / bpc;
        let chain = collect_cluster_chain_limited(&*self.disk, &self.bpb, cluster, nclusters);
        if chain.is_empty() { return Err(AbiError::Other("FAT32 no clusters")); }

        let mut copied = 0;
        let mut intra = skip;
        for chunk in chain.chunks(64) {
            let sector = self.bpb.cluster_to_sector(chunk[0]);
            let nsectors = chunk.len() * (self.bpb.sectors_per_cluster as usize);
            let nbytes = chunk.len() * bpc;
            let mut batch = vec![0u8; nbytes];
            self.disk.read_sectors(sector, nsectors as u32, &mut batch)
                .map_err(|_| AbiError::Other("FAT32 read I/O error"))?;
            let copy_len = (nbytes - intra).min(to_read - copied);
            buf[copied..copied + copy_len]
                .copy_from_slice(&batch[intra..intra + copy_len]);
            copied += copy_len;
            intra = 0;
        }
        Ok(copied)
