...
const COMMANDS: &[&str] = &[
    ...
    #[cfg(all(feature = "orbital", feature = "embed-orbital"))]
    "orbital",
    "exec", "kill",
...
impl Shell {
    ...
    pub fn run(&mut self) -> ! {
        ...
        crate::println!("[Diag Shell] interrupts_enabled = {}, cpu_id = {}, cpu_ticks = {}", 
            ...
            crate::arch::x86_64::per_cpu::current_cpu().cpu_id,
            crate::arch::x86_64::per_cpu::current_cpu().total_ticks());
    ...
    fn read_line(&mut self) {
        ...
        loop {
            ...
            if read_stdin(&mut byte) > 0 {
                ...
                // Yield to other tasks so they can run, but keep polling.
                crate::process::scheduler::yield_now();
                core::hint::spin_loop();
            }
        }
    ...
    fn cmd_orbital(&mut self, _args: &[String]) -> i32 {
        ...
        let binary = lz4_flex::decompress_size_prepended(compressed).unwrap_or_default();
        match crate::process::scheduler::spawn_redox_elf_from_vec(binary) {
            Some(pid) => {
