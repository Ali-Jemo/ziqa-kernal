...
fn frame_delay(ms: u64) {
    // ponytail: sleep most of the frame budget, yield the rest.
    // Old code spin_loop()'d the entire delay, starving other tasks.
    let deadline = crate::timer::uptime_ms().wrapping_add(ms);
    // Sleep for the bulk of the frame — leaves CPU free for other kthreads
    if ms > 2 {
        if let Some(pid) = crate::process::scheduler::with_current_task(|p| p.pid) {
            crate::timer::sleep_ms(pid, ms - 2);
        }
    }
    // Yield remaining ticks (at most ~2ms) instead of busy-spinning
