...
| **Host terminal echo looked like kernel input** | `run-gui.sh` now starts QEMU with a TCP-backed serial port, attaches it to the host terminal through `socat`, and restores terminal settings on exit instead of letting QEMU/GTK own stdio directly. |
...
| **TCP-attached QEMU GUI runner** | `make run-gui` launches GTK display plus a `socat`-attached serial shell, restores host terminal state on exit, and detaches with `Ctrl-]`. |
...
| **ABI Enhancement** | Added System V IPC syscall numbers (semget/semop/semctl, msgget/msgsnd/msgrcv, shmget/shmat/shmdt). WASI ABI now has sched_yield and random_get helpers integrated into the WASM interpreter. Added shm_open/shm_unlink for POSIX shared memory. All stubs return 0 until kernel IPC infrastructure is implemented. |
