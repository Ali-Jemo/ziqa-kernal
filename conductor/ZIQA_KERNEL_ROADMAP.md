...
*   **QEMU GUI Mode**: `make run-gui` opens GTK display while keeping the serial shell in the invoking terminal via raw `-serial stdio`. The runner restores terminal state on exit, and the foreground shell keeps timer preemption disabled while it owns command-line input.
