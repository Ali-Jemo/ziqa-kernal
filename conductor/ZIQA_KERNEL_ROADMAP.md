...
*   **Instant-On Resume**: Process snapshot save/restore through FAT32-backed storage. Full process state (registers, VMAs, FDs) serialized and restored at boot for instant-on resume.
*   **50ms Boot**: Optimized boot path reaches interactive shell in ~50ms on QEMU/KVM with spin-based APIC calibration and minimal debug output.
*   **Orbital GUI from Disk**: Orbital compositor loaded from FAT32 disk (`/fat/bin/orbital.elf`) or embedded compressed binary, with full capability grants for File, Memory, DeviceIo, and IPC.
---
...
*   **Boot-time Optimization**: Removed ~70% of debug prints from boot path, fixed APIC timer calibration bug (0-ticks → 12.5kHz flood), spin-based calibration producing proper ~73M count at 100Hz. Boot to shell: ~50ms.
*   **Process Snapshot & Instant-On Resume**: Binary snapshot format (v1) with ZSNP magic, serializes full process state (CPU registers, VMAs with page data, FD table, metadata). Restored at boot from FAT32-backed storage for instant-on resume.
*   **Orbital GUI from Disk**: Orbital compositor loaded from `/fat/bin/orbital.elf` on FAT32 disk or from embedded compressed binary (behind `orbital` feature). Full capability grants (File, Memory, DeviceIo, IpcChannel) for Orbital process.
*   **QEMU GUI Mode**: `make run-gui` opens GTK display while keeping the serial shell in the invoking terminal through a TCP serial socket attached by `socat`. The runner restores terminal state on exit, avoids direct QEMU/GTK ownership of stdio, and keeps the foreground shell in charge of command-line input.
*   **BGA Console Fallback**: If VirtIO GPU framebuffer setup fails, the compositor does not take over the display; the BGA framebuffer console remains active so QEMU GTK continues mirroring shell output.
