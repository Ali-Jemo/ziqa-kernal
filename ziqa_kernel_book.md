...
## الفصل 12: تطوير "الشل" وأدوات النظام (Shell & Utilities)

### 12.1 تطوير "الشل" (Enhancing the Shell)
تم تطوير "شل" ZiqaKernel (الموجود في `src/shell.rs`) و Bash (الموجود في `userspace/bash-5.2`) لدعم ميزات متقدمة:
- **`echo -N`**: خيار لطباعة النصوص متبوعة بـ Null Byte (`\0`) بدل المسافة، وهو مفيد جداً عند التعاون مع أدوات مثل `xargs -0`.
- **`cd -v`**: خيار "Verbose" يقوم بطباعة المسار الجديد للعمل بعد تغيير المجلد، مما يسهل تتبع الموقع داخل السكريبتات.
- **Autocompletion**: تم دمج ميزات الإكمال التلقائي في Bash باستخدام مكتبة `readline` لتسهيل كتابة الأوامر والمسارات.

### 12.2 مخطط الاستبطان (Introspection Schemes)
يعتمد ZiqaKernel على نظام "المخططات" (Schemes) لتوفير معلومات عن حالة النظام للمستخدم:
- **`sys:context`**: يوفر قائمة بجميع العمليات النشطة (PIDs)، حالتها، وعدد مناطق الذاكرة (VMAs) الخاصة بها.
- **`sys:memory`**: نقطة نهاية جديدة توفر إحصائيات الذاكرة (MemTotal, MemFree, MemUsed, HeapUsed).
- **`proc:<PID>/status`**: يوفر تفاصيل دقيقة عن عملية معينة.

### 12.3 أدوات النظام المستقلة (Standalone Utilities)
تمت إضافة أدوات مكتوبة بلغة C في مجلد `userspace/` تستخدم واجهة POSIX للتفاعل مع المخططات:
- **`ps`**: أداة تقوم بقراءة `sys:context` وعرض قائمة العمليات.
- **`free`**: أداة تقوم بقراءة `sys:memory` وعرض حالة الذاكرة بشكل مشابه للأنظمة الحديثة.

```c
// مثال بسيط لأداة free.c في ZiqaKernel
int main() {
...
## الفصل 13: توافقية Linux (Linux ABI Compatibility)

### 13.1 بنية الـ ABI (ABI Plugin Architecture)
يعتمد ZiqaKernel على نظام الإضافات (Plugins) لدعم واجهات برمجة التطبيقات المختلفة. يوفر `LinuxAbiPlugin` في `src/abi/linux/` التوافق مع تطبيقات Linux (x86_64).

### 13.2 تهيئة الذاكرة والـ Stack (Process Startup)
لكي تعمل تطبيقات Linux (مثل مكتبة `glibc`)، يجب أن يتم إعداد الـ Stack بنمط محدد (Linux x86_64 ABI):
1. **argc**: عدد الوسائط.
2. **argv**: قائمة بمؤشرات الوسائط (تنتهي بـ NULL).
3. **envp**: قائمة بمؤشرات متغيرات البيئة (تنتهي بـ NULL).
4. **auxv**: المتجه المساعد (Auxiliary Vector) الذي يوفر معلومات للنظام عن العتاد وخصائص التنفيذ.

تم تحديث `init_process_stack` في `src/arch/x86_64/mod.rs` ليقوم ببناء هذا الهيكل بدقة، مما يسمح للتطبيقات الثابتة (Static Binaries) بالعمل بشكل سليم.

### 13.3 دعم الروابط الديناميكية (Dynamic Linking Support)
تم تحسين محمل ملفات ELF (`src/abi/linux/elf_loader.rs`) ليتمكن من:
- التعرف على الجزء `PT_INTERP` واستخراج مسار الرابط الديناميكي (مثل `/lib64/ld-linux-x86-64.so.2`).
- دعم ملفات `ET_DYN` (التي يمكن تحميلها في أي مكان في الذاكرة) عن طريق تعيين قاعدة تحميل (Load Base) افتراضية عالية (`0x40_0000_0000`).

### 13.4 استدعاءات النظام (Syscall Implementation)
تم تنفيذ وتحسين مجموعة من استدعاءات النظام الحيوية لتوافق Linux:
- **`sys_fcntl`**: يدعم الآن `F_GETFL`, `F_SETFL`, و `F_DUPFD_CLOEXEC`.
- **`sys_poll`**: يدعم مراقبة الـ File Descriptors (حالياً stdin).
- **`sys_pipe2`**: يدعم إنشاء الأنابيب مع خيار `O_CLOEXEC`.
- **`sys_getdents64`**: يدعم سرد محتويات المجلدات لنظام الملفات Linux.
- **`sys_rt_sigaction`**: يدعم هيكل Linux الكامل للإشارات (56 بايت).

تسمح هذه التحسينات لـ ZiqaKernel بتشغيل تطبيقات Linux أكثر تعقيداً بشكل مباشر.
